...
export const crearArriendo = async (arriendo) => {
  try {
    const response = await fetch('http://node-server-service:5000/crear/nuevo/arriendo', {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
...
export const listarArriendos = async () => {
  try {
    const response = await fetch('http://node-server-service:5000/obtener/arriendo', {
      method: "GET",
    });
